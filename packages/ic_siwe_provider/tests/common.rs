...
pub const VALID_PUBKEY: &str = "5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpHgMR3n9bHKEq";
...
// Assuming `encode_one`, `update`, and similar functions are available and adapted for your use case
pub fn prepare_login_and_sign_message(
    ic: &PocketIc,
    ic_siwe_provider_canister: Principal,
    ...
    let signature = wallet.sign_message(siwe_message.as_bytes());

    // Convert the signature to a hex string
    let signature_str = bs58::encode(signature).into_string();

    (signature_str, siwe_message)
}
