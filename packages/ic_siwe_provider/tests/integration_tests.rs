...
#[test]
fn test_siwe_prepare_login_invalid_pubkey_legth() {
    let ic = PocketIc::new();
    let (ic_siwe_provider_canister, _) = init(&ic, None);
...
#[test]
fn test_siwe_prepare_login_invalid_pubkey() {
    let ic = PocketIc::new();
    let (ic_siwe_provider_canister, _) = init(&ic, None);
...
fn test_siwe_prepare_login_ok() {
    ...
    // assert_eq!(
    //     siwe_message.pubkey,
    //     hex::decode(&VALID_PUBKEY[2..]).unwrap().as_slice()
    // );
}
...
fn test_sign_in_message_expired() {
    ...
    let (signature, _) = prepare_login_and_sign_message(&ic, ic_siwe_provider_canister, &wallet);
...
fn test_sign_in_signature_manipulated() {
    ...
    let (ic_siwe_provider_canister, _) = init(&ic, None);
    let (wallet, pubkey) = create_wallet();
    let (_, _) = prepare_login_and_sign_message(&ic, ic_siwe_provider_canister, &wallet);
...
fn test_sign_in_siwe_get_delegation_timeout() {
    ...
    let (signature, _) = prepare_login_and_sign_message(&ic, ic_siwe_provider_canister, &wallet1);
    ...
    let (signature2, _) = prepare_login_and_sign_message(&ic, ic_siwe_provider_canister, &wallet2);
...
fn test_get_caller_address() {
    let ic = PocketIc::new();
    let (ic_siwe_provider_canister, targets) = init(&ic, None);
    let (pubkey, delegated_identity) = full_login(&ic, ic_siwe_provider_canister, targets);
...
fn test_get_principal_not_found() {
    ...
    assert_eq!(
        response.unwrap_err(),
        "No principal found for the given pubkey"
    );
}
...
fn test_principal_to_eth_mapping_disabled() {
    ...
    assert_eq!(
        response.unwrap_err(),
        "Principal to Solana pubkey mapping is disabled"
    );
}
