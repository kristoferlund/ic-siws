...
/// Represents the settings that determine the behavior of the SIWS library. It includes settings such as domain, scheme, statement,
...
/// Initialize the SIWS library with the given settings.
///
...
fn siws_init(settings_input: SettingsInput) {
    let mut ic_siws_settings = SettingsBuilder::new(
    ...
    if let Some(chain_id) = settings_input.chain_id {
        ic_siws_settings = ic_siws_settings.chain_id(chain_id);
    }
    if let Some(scheme) = settings_input.scheme {
        ic_siws_settings = ic_siws_settings.scheme(scheme);
    }
    if let Some(statement) = settings_input.statement {
        ic_siws_settings = ic_siws_settings.statement(statement);
    }
    if let Some(expire_in) = settings_input.sign_in_expires_in {
        ic_siws_settings = ic_siws_settings.sign_in_expires_in(expire_in);
    }
    if let Some(session_expire_in) = settings_input.session_expires_in {
        ic_siws_settings = ic_siws_settings.session_expires_in(session_expire_in);
    }
    if let Some(targets) = settings_input.targets {
        ...
        ic_siws_settings = ic_siws_settings.targets(targets);
    }
            ...
            for feature in runtime_features {
                match feature {
                    RuntimeFeature::IncludeUriInSeed => {
                        ic_siws_settings = ic_siws_settings.runtime_features(vec![
                            ic_siws::settings::RuntimeFeature::IncludeUriInSeed,
                        ]);
                    }
                    RuntimeFeature::DisableSolToPrincipalMapping => {
                        provider_settings.disable_sol_to_principal_mapping = true;
                    }
                    RuntimeFeature::DisablePrincipalToSolMapping => {
                        provider_settings.disable_principal_to_sol_mapping = true;
                    }
                }
            }
        }
...
/// `init` is called when the canister is created. It initializes the SIWS library with the given settings.
///
/// Required fields are `domain`, `uri`, and `salt`. All other fields are optional.
///
/// ## 🛑 Important: Changing the `salt` or `uri` setting affects how user seeds are generated.
...
/// `post_upgrade` is called when the canister is upgraded. It initializes the SIWS library with the given settings.
///
/// Required fields are `domain`, `uri`, and `salt`. All other fields are optional.
///
/// ## 🛑 Important: Changing the `salt` or `uri` setting affects how user seeds are generated.
