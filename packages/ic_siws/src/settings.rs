...
///
/// This builder provides a flexible way to configure and initialize the settings for SIWS (Sign-In With Solana).
...
///
/// ```
/// use ic_siws::settings::{Settings, SettingsBuilder};
///
...
impl SettingsBuilder {
    ...
    /// # Parameters
    ///
    /// * `domain`: The domain from where the frontend that uses SIWS is served.
    /// * `uri`: The full URI, potentially including port number of the frontend that uses SIWS.
    ...
    pub fn chain_id<S: Into<String>>(mut self, chain_id: S) -> Self {
        self.settings.chain_id = chain_id.into();
        self
    }

    /// The scheme used to serve the frontend that uses SIWS.
    ...
    /// The `statement` is a message or declaration, often presented to the user by the Solana wallet
    /// during the sign-in process. Defaults to "SIWS Fields:".
    ...
    /// Optional runtime features customize the behavior of ic-siws.
...
mod tests {
    ...
    fn test_multiple_valid_and_invalid_combinations() {
        let builder = SettingsBuilder::new("", "invalid_uri", "").scheme("ftp");
        assert!(builder.build().is_err());
    }
